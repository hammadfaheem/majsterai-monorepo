...
class Organization(Base):
    ...
    va_phone: Mapped["VirtualAssistantPhone | None"] = relationship(
...
class Agent(Base):
    ...
    va_phones: Mapped[list["VirtualAssistantPhone"]] = relationship(
...
class VirtualAssistantPhone(Base):
    ...
    # Org / agent assignment (one phone per org enforced by unique constraint on org_id)
    ...
    # Twilio Elastic SIP Trunk fields (populated in Phase 3 / register)
    ...
    # LiveKit SIP fields (populated in Phase 4 / bind)
    ...
    # Optional per-org LiveKit credential overrides for multi-tenant setups
    ...
    agent: Mapped["Agent"] = relationship("Agent", back_populates="va_phones")
